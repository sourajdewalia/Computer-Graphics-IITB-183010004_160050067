...
void sphere(double radius, int Lats, int Longs)
{
  ...
  float slices = PI / 100.0;
  float sectors = PI / 100.0;
  ...
  for (lats = 0.0; lats <= PI; lats += sectors)
  {
    for (longs = 0.0; longs <= 2.0 * PI + slices; longs += slices)
    {
      ...
      //glm::vec2 tex_pos;
      glm::vec2 tex_pos = glm::vec2(longs / (2.0 * PI), 1.0 - lats / PI);
      /*if (alter)
      ...
      tex_coords[tri_idx] = tex_pos;
      tri_idx++;

      //      w_colors[wire_idx] = black;
      //      w_positions[wire_idx] = pt;
      //      w_normals[wire_idx] = pt;
      //      wire_idx++;
      ...
      z = radius * cos(l);
      pt = glm::vec4(x, y, z, 1.0f);
      tex_pos = glm::vec2(longs / (2.0 * PI), 1.0 - l / PI);
      /*if (alter)
...
void initBuffersGL(void)
{
  ...
  cubePtr = new ObjModel("../images/cube.obj");
  /*
  ...
  // Load Textures
  tex = LoadTexture("../images/1_earth_8k.bmp");
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(v_positions) + sizeof(v_colors) + sizeof(tex_coords) + sizeof(v_normals), NULL, GL_STATIC_DRAW);
  glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(v_positions), v_positions);
  glBufferSubData(GL_ARRAY_BUFFER, sizeof(v_positions), sizeof(v_colors), v_colors);
  glBufferSubData(GL_ARRAY_BUFFER, sizeof(v_colors) + sizeof(v_positions), sizeof(tex_coords), tex_coords);
  glBufferSubData(GL_ARRAY_BUFFER, sizeof(v_colors) + sizeof(v_positions) + sizeof(tex_coords), sizeof(v_normals), v_normals);
  // set up vertex array
  ...
  glVertexAttribPointer(vTexCoords, 2, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(sizeof(v_positions) + sizeof(v_colors)));

  glEnableVertexAttribArray(vNormal);
  glVertexAttribPointer(vNormal, 4, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(sizeof(v_positions) + sizeof(v_colors) + sizeof(tex_coords)));
*/
...
void renderGL(void)
{
  ...
  projection_matrix = glm::frustum(-1.0, 1.0, -1.0, 1.0, 2.0, 200.0);
